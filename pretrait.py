...
def load_insee_data(filepath, encoding='utf-8'):
    ...
    try:
        ...
        log_message("INFO: Données INSEE chargées avec succès.")
    except UnicodeDecodeError:
        log_message(f"ERROR: Erreur de décodage avec l'encodage {encoding}.")
    except FileNotFoundError:
        log_message(f"FATAL: Fichier INSEE '{filepath}' introuvable.")
        raise SystemExit(1)
    except Exception as e:
        log_message(f"ERROR: Problème lors du chargement des données INSEE - {e}")
    return insee_data
...
def maj_addr(row, insee_data):
    ...
    try:
        ...
        log_message(f"ERROR: Clé manquante dans les données de la ligne - {e}")
...
def preprocess_csv(file_path, source):
    ...
    try:
        ...
        df['source'] = source
        log_message(f"INFO: Chargement du fichier '{file_path}' terminé avec succès.")
        return df
    except FileNotFoundError:
        log_message(f"FATAL: Le fichier '{file_path}' est introuvable.")
        raise SystemExit(1)
    except pd.errors.ParserError as e:
        log_message(f"ERROR: Erreur de parsing lors du chargement du fichier '{file_path}' - {e}")
        return pd.DataFrame()  # Return an empty DataFrame to allow the program to continue
    except Exception as e:
        log_message(f"ERROR: Problème lors du chargement du fichier '{file_path}' - {e}")
...
def determine_action(row):
    ...
    try:
        ...
        log_message(f"ERROR: Clé manquante pour déterminer l'action - {e}")
        return "UNKNOWN"
...
def merge_and_process(added_path, modified_path, removed_path, output_path, insee_data):
    ...
    try:
        ...
        if added_df.empty or modified_df.empty or removed_df.empty:
            log_message(f"ERROR: Un ou plusieurs fichiers de données sont vides ou n'ont pas été chargés correctement.")
            return
        ...
        log_message(f"INFO: Fichier final '{output_path}' généré avec succès.")
    except Exception as e:
        log_message(f"FATAL: Échec lors du traitement des fichiers - {e}")
        raise SystemExit(1)
...
def main(no_insee, no_process, debug):
    ...
    if not no_insee:
        ...
        log_message('WARN: Chargement INSEE sauté : demandé par argument')
        insee_data = {}

    if not no_process:
        merge_and_process(added_path, modified_path, removed_path, pretraite_path, insee_data)
        log_message('INFO: Prétraitement terminé')
    else:
        log_message('WARN: Prétraitement sauté : demandé par argument')
