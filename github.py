...
def del_file_repo(local_dir, filename):
    ...
    if os.path.exists(file_to_del):
        ...
        log_message(f"INFO: Fichier {file_to_del} supprimé.")
    else:
...
def main(no_del_clone, no_del_file, no_copy_file, no_commit, no_push, debug):
    ...
    if not no_del_clone:
        ...
        log_message("INFO: Repo supprimé puis cloné avec succès.")
    else:
        log_message(f" La suppression du repo ainsi que son clonage ont été sautés : demandé par argument.")
        log_message("FATAL: Si cette étape est sautée, le repo n'est pas initialisé. Impossible de continuer.")
        sys.exit(1)
    ...
    if not no_del_file:
        ...
        log_message(f" Suppression du fichier dans le repo sautée : demandé par argument.")
    ...
    if not no_copy_file:
        ...
        log_message(f" Copie du/des nouveaux fichier(s) dans le repo sautée : demandé par argument. Vous n'allez donc rien 'commiter' ?")

    # 4. Commit les modifications
    if not no_commit:
        commit_modif(repo)
    else:
        log_message(f" Commit sauté : demandé par argument. Vous n'allez donc rien pousser ?")

    # 5. Pousser les modifications vers GitHub
    if not no_push:
        push_to_github(repo)
    else:
        log_message(f" Push sauté : demandé par argument.")
